    ...
    <div className="flex min-h-screen bg-background">
      ...
      <main className="md:ml-[185px] mt-14 md:mt-0 flex-1 min-h-screen overflow-y-auto">
        <div className="max-w-[900px] mx-auto px-4 md:px-8 py-8 md:py-10">
          ...
          {/* Active system */}
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14, duration: 0.35 }} className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[15px] font-bold text-foreground">Active system</h2>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {[
                { name: "gitShield", desc: "Lock assets into vault", status: "LIVE" },
                { name: "gitSwap", desc: "On-chain USDC / WETH swap", status: "LIVE" },
                { name: "gitSend", desc: "Vault-to-vault transfer", status: "LIVE" },
                { name: "GitBounty", desc: "Auto-payout on PR merge", status: "LIVE" },
                { name: "AutoGit", desc: "AI app builder to GitHub Pages", status: "LIVE" },
                { name: "gitNeo", desc: "Crypto neobank virtual card", status: "BUILDING" },
              ].map(sys => (
                <div key={sys.name} className="rounded-xl border border-border bg-card p-4 flex flex-col gap-2">
                  <div className="flex items-center justify-between gap-1 flex-wrap">
                    <p className="text-[12px] font-bold text-foreground font-mono">{sys.name}</p>
                  ...
                  <p className="text-[11px] text-muted-foreground leading-snug">{sys.desc}</p>
                </div>
              ))}
            </div>
          </motion.div>
