...
import {
  Home, Database, Map, Users, Sun, Moon, X, Layers,
  BookOpen, Github, Cpu, Zap,
} from "lucide-react";

type NavItem = { label: string; icon: ElementType; href: string; badge?: string; badgeVariant?: "live" | "install" | "new" | "building" };
...
const navGroups: NavGroup[] = [
  ...
  },
  {
    section: "TOOLS",
    items: [
      { label: "GitHub App", icon: Github,   href: "/github-app", badge: "INSTALL",  badgeVariant: "install" },
      { label: "MCP",        icon: Cpu,      href: "/mcp",        badge: "LIVE",     badgeVariant: "live" },
      { label: "x402",       icon: Zap,      href: "/x402" },
    ],
  },
  {
    section: "PLATFORM",
    items: [
      ...
      { label: "RWA",        icon: Layers,   href: "/docs#rwa",   badge: "BUILDING", badgeVariant: "building" },
...
const BADGE_STYLES: Record<string, string> = {
  ...
  install:  "bg-blue-500/15 text-blue-500",
...
function SidebarStats() {
  ...
  useEffect(() => {
    ...
    fetch("/api/stats")
      .then(r => r.json())
      .then(d => setVaults(d?.vaultsDeployed ?? null))
      .catch(() => {});
  }, []);

  return (
    <div className="mx-3 mb-3 rounded-lg border border-border bg-muted/20 px-3 py-2.5 flex flex-col gap-1.5">
      <div className="flex items-center gap-1.5">
        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse flex-shrink-0" />
        <span className="text-[9px] font-bold tracking-widest uppercase text-muted-foreground">Base Mainnet</span>
      </div>
      <div className="flex items-baseline gap-1">
        <span className="text-[15px] font-bold font-mono text-foreground">
          {vaults !== null ? vaults.toLocaleString() : "--"}
        </span>
        <span className="text-[10px] font-mono text-muted-foreground">vaults deployed</span>
      </div>
    </div>
  );
}
...
/* ── Nav content ─────────────────────────────────────────────── */
function NavContent({ onNavigate, open = true }: { onNavigate?: () => void; open?: boolean }) {
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {item.badge && open && (
                        <span className={`text-[9px] font-semibold tracking-wide px-1.5 py-0.5 rounded-full leading-none ${
                          BADGE_STYLES[item.badgeVariant ?? "live"] ?? BADGE_STYLES.live
                        }`}>
                          {item.badge}
                        </span>
                      )}
      ...
      <div className="hidden md:block">
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden" style={{ width: SIDEBAR_W }}>
            <NavContent onNavigate={snapClosed} open={open} />
          </div>
